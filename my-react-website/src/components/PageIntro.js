...
/**
 * Alamo Drafthouse-style intro — plays once per session.
 * Black full-screen. "THE" slams in, then "INTERESTING GROUP",
...
export default function PageIntro({ onDone }) {
  ...
  useEffect(() => {
    // sequence timing
    const t1 = setTimeout(() => setPhase('period'), 1000);  // period starts appearing
    const t2 = setTimeout(() => setPhase('out'),    2600);  // whole thing fades out
    const t3 = setTimeout(() => { setPhase('gone'); onDone(); }, 3400); // unmount
